...
import { LogOut, Hexagon, Wifi, Cloud, CloudOff } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  // Real-time Sync Logic with Firebase
  useEffect(() => {
  ...
  const handleCreateJob = (newJob: JobCard) => {
    addJobToFirebase(newJob);
  };

  const handleUpdateJob = (updatedJob: JobCard) => {
    updateJobInFirebase(updatedJob);
  };
            ...
            <div>
              <h1 className="text-lg font-bold leading-none tracking-tight flex items-center gap-2">
                ...
                <span className="flex items-center gap-1 text-[10px] bg-white/10 border border-white/20 px-1.5 py-0.5 rounded uppercase tracking-wider font-bold text-white/90">
                   ...
                   {isConnected ? 'Cloud Live' : 'Offline'}
                </span>
              </h1>
